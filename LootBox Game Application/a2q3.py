...
def open_Box(amountOfBoxes, typeOfBox, nameOfBox):
    common_Count = 0
    rare_Count = 0
    epic_Count = 0
    for i in range(amountOfBoxes):
        for j in range(3):
            ...
            if(random_Number <= typeOfBox[0]):
                #print("COMMON ITEM " + str(random_Number))
                common_Count += 1

            elif(random_Number <= (typeOfBox[0]+typeOfBox[1])):
                #print("RARE ITEM "+str(random_Number))
                rare_Count += 1

            else:
                #print("EPIC ITEM "+str(random_Number))
                epic_Count += 1

    print("\n{}({}x) BOX ITEMS:"
              "\nCommon: {}"
              "\nRare: {}"
              "\nEpic: {}".format(nameOfBox, amountOfBoxes,common_Count, rare_Count, epic_Count))

if(amount_GroundHog > 0):
    open_Box(amount_GroundHog, COMMON, groundHog_Name)

if(amount_Squirrel > 0):
    open_Box(amount_Squirrel, RARE, squirrel_Name)

if(amount_Raven > 0):
    open_Box(amount_Raven, EPIC, raven_Name)
